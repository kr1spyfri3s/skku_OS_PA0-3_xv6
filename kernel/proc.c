...
const int nice_to_weight[40] = {
    88761, 71755, 56483, 46273, 36291, // *0
...
found:
  ...
  p->runtime = 0;
  p->vruntime = 0;
  p->vdeadline = p->vruntime + (5000 * 1024) / nice_to_weight[p->nice];
  p->time_slice = 5;
...
kfork(void)
{
  ...
  *(np->trapframe) = *(p->trapframe);
  ...
  np->vdeadline = np->vruntime + (5000 * 1024) / nice_to_weight[np->nice];
...
void scheduler(void) {
  ...
  for(;;){
    ...
    if (min_vruntime != (uint64)-1) {
      for(p = proc; p < &proc[NPROC]; p++) {
        acquire(&p->lock);
        if(p->state == RUNNABLE || p->state == RUNNING) {
          uint64 w_i = nice_to_weight[p->nice];
          sum_weight += w_i;
          left_term += (p->vruntime - min_vruntime) * w_i;
        }
        release(&p->lock);
      }
    }
    ...
    for(p = proc; p < &proc[NPROC]; p++) {
      ...
      if(p->state == RUNNABLE) {
        ...
        uint64 right_term = (p->vruntime - min_vruntime) * sum_weight;
...
//    if(p->state != UNUSED) {
//      if(pid == 0 || p->pid == pid) {
//        printf("%s\t%d\t%s\t%d\n", p->name, p->pid, state_names[p->state], p->nice);
//      }
//    }
//    release(&p->lock);
//  }
  ...
  for(p = proc; p < &proc[NPROC]; p++) {
    ...
    if(p->state != UNUSED) {
      if(pid == 0 || p->pid == pid) {
        ...
        uint64 rt_milli = p->runtime * 1000;
        uint64 rw = rt_milli / nice_to_weight[p->nice];

        char *elig = "false";

        if(min_vruntime == (uint64)-1 || p->vruntime <= min_vruntime) {
          elig = "true";
        } else {
          uint64 right_term = (p->vruntime - min_vruntime) * sum_weight;
          if(left_term >= right_term) {
        ...
        printf("%s\t%d\t%s\t%d\t\t%d\t\t%d\t%d\t\t%d\t\t%s\n",
               p->name, p->pid, state_names[p->state], p->nice,
               (int)rw, (int)rt_milli, (int)p->vruntime, (int)p->vdeadline, elig);
      }
    }
    release(&p->lock);
  }
...
// Update EEVDF time on every timer tick
void update_eevdf_time(void) {
  struct proc *p = myproc();
  ...
  p->vruntime += (1000 * 1024) / nice_to_weight[p->nice];
  ...
  if(p->time_slice <= 0) {
    // Recalculate virtual deadline
    p->vdeadline = p->vruntime + (5000 * 1024) / nice_to_weight[p->nice];

    // Refill time slice (default : 5 ticks)
    p->time_slice = 5;
